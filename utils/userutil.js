...
async function getUserFromToken(req, show_passwd = false){
    try {
        const jwt_dc = jwt.decode(req.headers['authorization'], process.env.SECRET_JWT)
          ...
          let result = await db.query(`SELECT * FROM user_detail WHERE ${jwt_dc.email ? "auth_gg_email" : "username"} = $1`, [jwt_dc.email ? jwt_dc.email : jwt_dc.sub]);
...
async function getUserFromUID(uid, show_passwd = false){
    try {
          let result = await db.query('SELECT * FROM user_detail WHERE uid = $1', [uid]);
...
async function getUserRole(user_data){
    try {
          let result = await db.query('SELECT * FROM user_additionrole WHERE uid = $1', [user.uid]);
    ...
    return {
      user: {
        ...
        study_status: {
          ...
          courseset: courseset_res.rowCount == 0 ? null : {...courseset_res.rows[0]},
        }
      }
    }
  } else {
...
async function getUserFromGoogle(email){
  const result = await db.query('SELECT * FROM user_detail WHERE auth_gg_email = $1', [email]);
  if(result.rows.length > 0){
    let user_nopass = {...result.rows[0], password:undefined}
