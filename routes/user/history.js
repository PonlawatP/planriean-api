...
async function getUserSubjectHistory(req, res){
  try {
      ...
      const {id} = req.params
      if(user != null){
        const crs = await db.query('SELECT * FROM user_subjecthistory WHERE uid = $1;', [user.uid]);
        const history_head = []
...
async function updateUserSubjectHistory(req, res){
  try {
      ...
      if(user != null){
        ...
        await db.query(`DELETE FROM user_subjecthistory WHERE uid = $1;`, [user.uid]);
        // เอาข้อมูลบางส่วนที่ไม่ได้ส่งมากับ body ยัดใส่ก่อนสร้าง sql placeholder
        const sql_placeholders = subj_sql.map((_, i) => `(${Object.keys(subj_sql[0]).map((_, j) => `$${i * Object.keys(subj_sql[0]).length + j + 1}`).join(',')})`).join(',');
        console.log(sql_placeholders)
        const insertQuery = `
            INSERT INTO "user_subjecthistory" (uid, subj_id, study_term, study_year, grade, score, uni_id)
            VALUES ${sql_placeholders}
        ;`;
        const values = subj_sql.flatMap(subject => [subject.uid, subject.subj_id, subject.study_term, subject.study_year, subject.grade, subject.score, subject.uni_id]);
